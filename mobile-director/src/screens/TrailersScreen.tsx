...
import { fetchTrailers } from "../api/client";
...
type Props = NativeStackScreenProps<RootStackParamList, "Trailers">;
...
export function TrailersScreen({ navigation }: Props) {
  ...
  const filtered = trailers.filter((t) => t.plateNumber.toLowerCase().includes(query.toLowerCase()));
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<EmptyState text="Aucune remorque." />}
        renderItem={({ item }) => (
          <Pressable style={styles.row} onPress={() => navigation.navigate("TrailerForm", { trailerId: item.id })}>
            <View style={styles.rowInfo}>
              <Text style={styles.rowTitle}>{item.plateNumber}</Text>
              <Text style={styles.rowSubtitle}>
                {TYPE_LABELS[item.type] ?? item.type}
                {item.capacityTons ? ` · ${item.capacityTons} t` : ""}
              </Text>
            </View>
      ...
      />
      <FAB onPress={() => navigation.navigate("TrailerForm", {})} />
    </View>
  );
}
